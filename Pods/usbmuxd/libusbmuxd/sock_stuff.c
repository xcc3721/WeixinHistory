...
int create_unix_socket(const char *filename)
{
	...
	if (bind(sock, (struct sockaddr *) &name, (int)size) < 0) {
		perror("bind");
		close_socket(sock);
		return -1;
	}
...
int connect_unix_socket(const char *filename)
{
	...
	if (connect(sfd, (struct sockaddr *) &name, (int)size) < 0) {
		close_socket(sfd);
		if (verbose >= 2)
					 ...
					 unsigned int timeout)
{
	...
	result = (int)recv(fd, data, length, flags);
	if (res > 0 && result == 0) {
...
{
	return (int)send(fd, data, length, 0);
}
