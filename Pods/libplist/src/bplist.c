    ...
    for (i = 0; i < num_objects; i++)
    {
        ...
        case PLIST_DICT:
            for (j = 0; j < data->length; j++)
            {
                ...
                str_j = (int)(j + data->length) * dict_param_size;

                index1 = (int)UINT_TO_HOST(data->buff + str_i, dict_param_size);
                index2 = (int)UINT_TO_HOST(data->buff + str_j, dict_param_size);
        ...
        case PLIST_ARRAY:
            for (j = 0; j < data->length; j++)
            {
                ...
                index1 = (int)UINT_TO_HOST(data->buff + str_j, dict_param_size);
...
void plist_to_bin(plist_t plist, char **plist_bin, uint32_t * length)
{
    ...
    for (i = 0; i < num_objects; i++)
    {
        ...
        case PLIST_STRING:
            ...
            if ( is_ascii_string(data->strval, (int)len) )
            {
