...
int socket_create_unix(const char *filename)
{
	...
	if (bind(sock, (struct sockaddr *) &name, (uint)size) < 0) {
		perror("bind");
		socket_close(sock);
		return -1;
	}
...
int socket_connect_unix(const char *filename)
{
	...
	if (connect(sfd, (struct sockaddr *) &name, (uint)size) < 0) {
		socket_close(sfd);
		if (verbose >= 2)
...
int socket_check_fd(int fd, fd_mode fdm, unsigned int timeout)
{
	...
	if (timeout > 0) {
		...
		to.tv_usec = (int)(time_t) ((timeout - (to.tv_sec * 1000)) * 1000);
		pto = &to;
	} else {
		pto = NULL;
	}
					 ...
					 unsigned int timeout)
{
	...
	result = (uint)recv(fd, data, length, flags);
	if (res > 0 && result == 0) {
...
{
	return (uint)send(fd, data, length, 0);
}
