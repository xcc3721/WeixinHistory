...
static afc_error_t afc_dispatch_packet(afc_client_t client, const char *data, uint32_t length, uint32_t *bytes_sent)
{
	...
	*bytes_sent = 0;
	...
	if (client->afc_packet->this_length != client->afc_packet->entire_length) {
		offset =(uint)client->afc_packet->this_length - sizeof(AFCPacket);
...
afc_error_t afc_read_directory(afc_client_t client, const char *dir, char ***list)
{
	...
	ret = (uint)afc_dispatch_packet(client, dir, (uint)strlen(dir)+1, &bytes);
...
afc_error_t afc_remove_path(afc_client_t client, const char *path)
{
	...
	ret = afc_dispatch_packet(client, path, (uint)strlen(path)+1, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
...
afc_error_t afc_rename_path(afc_client_t client, const char *from, const char *to)
{
	...
	ret = afc_dispatch_packet(client, send, (uint)strlen(to)+1 + (uint)strlen(from)+1, &bytes);
	free(send);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
afc_error_t afc_make_directory(afc_client_t client, const char *dir)
{
	...
	client->afc_packet->this_length = client->afc_packet->entire_length = 0;
	ret = afc_dispatch_packet(client, dir, (uint)strlen(dir)+1, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
					 ...
					 afc_file_mode_t file_mode, uint64_t *handle)
{
	...
	*handle = 0;
	...
	client->afc_packet->entire_length = client->afc_packet->this_length = 0;
	ret = afc_dispatch_packet(client, data, 8 + (uint)strlen(filename) + 1, &bytes);
	free(data);
...
afc_error_t afc_truncate(afc_client_t client, const char *path, uint64_t newsize)
{
	...
	ret = afc_dispatch_packet(client, send, 8 + (uint)strlen(path) + 1, &bytes);
	free(send);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
afc_error_t afc_make_link(afc_client_t client, afc_link_type_t linktype, const char *target, const char *linkname)
{
	...
	ret = afc_dispatch_packet(client, send, 8 + (uint)strlen(linkname) + 1 + (uint)strlen(target) + 1, &bytes);
	free(send);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
